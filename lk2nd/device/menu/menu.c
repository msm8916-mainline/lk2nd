...
void display_fastboot_menu(void)
{
	...
	if (lk2nd_dev.single_key) {
		...
		if(lk2nd_dev.menu_keys.navigate)
			fbcon_printf_ln(SILVER, y, incr, true, "%s to navigate.", lk2nd_dev.menu_keys.navigate);
		else
			fbcon_puts_ln(SILVER, y, incr, true, "Volume keys to navigate.");

		if(lk2nd_dev.menu_keys.select)
			fbcon_printf_ln(SILVER, y, incr, true, "%s to select.", lk2nd_dev.menu_keys.select);
		else
