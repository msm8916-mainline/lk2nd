...
static int lk2nd_audio_q6afe_to_lpass(uint32_t id)
{
	switch (id) {
...
{
	enum rproc_mode mode = RPROC_MODE_UNKNOWN;
	char args[16];

	if (!lk2nd_cmdline_scan_arg(cmdline, "lk2nd.rproc-mode", args, sizeof(args)))
		return 0;

	if (!strcmp(args, "all"))
		mode = RPROC_MODE_ALL;
	else if (!strcmp(args, "no-modem"))
		mode = RPROC_MODE_NO_MODEM;
	else if (!strcmp(args, "none"))
		mode = RPROC_MODE_NONE;

	if (mode <= RPROC_MODE_ALL)
		/* Just keep everything as-is for now */
		return 0;

	lk2nd_disable_rprocs(dtb, mode);
